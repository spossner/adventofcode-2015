...
def cheapest_winning_spell_sequence(player_hitpoints=0, player_mana=0,
    ...
    while True:
        ...
        if you.is_alive() and ((you.health // 3) + 1 >= (boss.health // 6)):
            if boss.health < 13:
                print
                "Cost: {:4d}  You: {} hp, {} mana  Boss: {} hp  Spells: {}".format(
...
if __name__ == '__main__':
    ...
    for cost, inventory in results[:5]:
        print("Cost: {}  Inventory: {}".format(cost,", ".join(item.name for item in inventory)))
    ...
    spell_sequence = [SHIELD_SPELL, RECHARGE_SPELL, POISON_SPELL, SHIELD_SPELL, RECHARGE_SPELL, POISON_SPELL, SHIELD_SPELL, RECHARGE_SPELL,
                      POISON_SPELL, SHIELD_SPELL, DRAIN_SPELL, POISON_SPELL, MISSILE_SPELL]
    run_scenario(you, boss, spell_sequence)
    report_winner(you, boss)
    print()
